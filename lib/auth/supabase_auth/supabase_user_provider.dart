...
class DuplicateHarshSupabaseUser extends BaseAuthUser {
  DuplicateHarshSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> duplicateHarshSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DuplicateHarshSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
